...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    # Managed slots are identified by their commit pointer, not by the
    # archived app tree (which intentionally has no .git/install stamp).
    if (hermes_home / "current.txt").is_file():
        if not updater.is_file():
            print(f"✗ Managed updater is missing: {updater}")
            sys.exit(1)
        command = [str(updater), "apply"]
        if gateway_mode:
            command += ["--report", "json"]
    ...
    # Git checkout: auto-adopt pristine trees; otherwise use worktrees only.
    if method == "git" and git_dir.exists():
        # ── Auto-adopt: pristine clean-main checkouts → bundled slots ──
        try:
            from hermes_cli.adoption import detect_legacy_install

            legacy = detect_legacy_install(PROJECT_ROOT, hermes_home)
            if legacy is not None and legacy.pristine:
        ...
        if not should_use_worktree_update(PROJECT_ROOT, in_place=False):
            print("✗ Worktree updates are unavailable for this checkout.")
            sys.exit(1)
        branch = _resolve_update_branch(args)
        input_fn = (
    ...
    print(f"✗ Unsupported install method: {method}")
    sys.exit(1)
