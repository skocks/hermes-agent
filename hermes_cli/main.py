...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    def _run(cmd: list[str]) -> subprocess.CompletedProcess:
        ...
        def _attempt_relaxed(npm_exe: str) -> subprocess.CompletedProcess:
            ...
            lockfile = cwd / "package-lock.json"
            if lockfile.exists():
                ci_result = _run([npm_exe, "ci", "--include=dev", "--engine-strict=false", *extra_args])
                if ci_result.returncode == 0:
                    return ci_result
            return _run([npm_exe, "install", "--no-save", "--include=dev", "--engine-strict=false", *extra_args])
